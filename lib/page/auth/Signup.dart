...
class _SignupState extends State<Signup> {
  ...
  signup() {
    ...
    auth
        .postSign({"account": account, username: username, "password": sha256.convert(utf8.encode(password)).toString(), "email": email})
        .then((value) => {
              if (value["code"] == 0)
                {
                  showDialog(
    ...
    return Scaffold(
      body: ListView(
        ...
        children: [
          ...
          ElevatedButton(onPressed: () => {Navigator.pop(context)}, child: Text("返回登录")),
        ],
      ),
    );
  }
